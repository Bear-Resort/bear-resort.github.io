  ...
  if (name.trim() === '') {
    ...
    if (updateMyLanguage() === 'Eng') {
      document.getElementById('result').textContent =
        'You need to fill in your name, my friend! We do not record your name and it is for your own reference.';
    } else {
      document.getElementById('result').textContent =
        '你需要填写你的名字，我的朋友。我们不会记录你的名字，这个是给你自己的.';
    }
  } else {
    var outcome = '';
    var sentence = '';
    ...
    if (updateMyLanguage() === 'Eng') {
      ...
      document.getElementById('result').textContent =
        'Your Hopkins life is identified as ' + outcome + '. ' + sentence;
      document.getElementById('end').textContent = 'Sincerely, Bear Resort.';
    } else {
      document.getElementById('greeting').textContent = '嘿' + name + ',';
      document.getElementById('result').textContent =
        '你的霍普金斯生活被鉴定为' + outcome_ch + '. ' + sentence_ch;
      document.getElementById('end').textContent = '诚挚的, 小熊樂園.';
    }
  }
